...
# Database connection function (as before)
@st.cache_resource
def init_connection():
...
if not st.session_state.logged_in:
    ...
    if page == "Drivers":
        ...
        # Display fastest lap and pole position for the circuit
        circuit_results = run_query(f"""
            SELECT 
                CONCAT(d1.Fname, ' ', d1.Lname) as FastestLapDriver,
                fl.Time as FastestLapTime,
                CONCAT(d2.Fname, ' ', d2.Lname) as PoleDriver,
                pp.Q_Time as PoleTime
            FROM circuit c
            LEFT JOIN fastest_lap fl ON c.Circuit_ID = fl.Circuit_ID
            LEFT JOIN driver_list d1 ON fl.Driver_ID = d1.Driver_ID
            LEFT JOIN pole_position pp ON c.Circuit_ID = pp.Circuit_ID
            LEFT JOIN driver_list d2 ON pp.Driver_ID = d2.Driver_ID
            WHERE c.Circuit_ID = '{selected_circuit_id}'
        """)
        if circuit_results:
            df_results = pd.DataFrame(circuit_results, columns=["Fastest Lap Driver", "Fastest Lap Time", "Pole Driver", "Pole Time"])
